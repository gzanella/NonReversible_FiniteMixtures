rev_sampler_highdim <- function(alpha, n, K, iterations, c, y, sigma2, mu0, sigma20){
  ...
  sums_y <- matrix(0, K, p)
  for(k in 1:K){
    sums_y[k,] <- colSums(as.matrix(y[c == k,]))
  }
  ...
  for(t in 1:iterations){
    ...
    means <- matrix(0, K, p)
    for(j in 1:p){
      means[,j] <- sigma20/(sigma2+sums*sigma20)*sums_y[,j] + sigma2/(sigma2+sums*sigma20)*mu0
    }
    ...
    for(j in 1:p){
      log_prob <- log_prob + dnorm(y_i[j], means[,j], sqrt(vars), log = T)
    }
...
nonrev_sampler_highdim <- function(alpha, n, K, iterations, c, y, sigma2, mu0, sigma20, verbose = T){
  #K = number of clusters
  #alpha = K dimensional vector
  #sigma2 = variance of the likelihood
  #mu0 = mean of the baseline measure
  #sigma20 = variance of the baseline
  #change_vel = rate of change of velocity
  #y = n x p matrix
  ...
  for(t in 1:iterations){
    ...
    if(n_minus > 0){
      ...
      #likelihood
      log_alpha <- log_alpha + 
        sum(dnorm(y_i, mean_plus, sqrt(var_plus), log = T) - dnorm(y_i, mean_minus, sqrt(var_minus), log = T))
...
exact_sample_highdim <- function(probs, n, K, mu, sigma2){
  #mu = K x p matrix
  ...
  #sample observations
  y <- mu[comp] + sqrt(sigma2[comp])*matrix(rnorm(n*p), n, p)
  return(y)
}